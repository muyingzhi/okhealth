...
angular.module('xc.indexedDB', []).provider('$indexedDB', function() {
    ...
    module.$get = ['$q', '$rootScope', '$window', function($q, $rootScope, $window) {
        ...
        if(!($window.indexedDB)){alert("浏览器不支持indexedDB，不能离线随访。")}
        ...
        ObjectStore.prototype = {
            ...
            /**
             * @ngdoc method
             * @name ObjectStore.modifyByKey
             * @description 通过先删除后增加的办法实现修改数据的操作；
             */
            "modifyByKey" : function(data,key){
                var d = $q.defer();
                var _this = this;
                this.delete(key).then(function(){
                    _this.upsert(data).then(function(result){
                        d.resolve(result);
                    })
                })
                return d.promise;
            ...
            "each": function(callback, options){
                ...
                return this.internalObjectStore(this.storeName, READWRITE).then(function(store){
                   var req;
                   var option = options.result || defaultQueryOptions;
                   if(option.useIndex) {
                        req = store.index(option.useIndex).openCursor(option.keyRange, option.direction);
                    } else {
                        req = store.openCursor(option.keyRange, option.direction);
                    }
                    req.onsuccess = req.onerror = function(e) {
                        $rootScope.$apply(function(){
                            if(!e.target.result){
                ...
                return dbPromise().then(function(db){
                    ...
                    storeNames.forEach(function(storeName){
                        ...
                        for(var j=0;j<store.indexNames.length;j++){
                            var idx = store.index(store.indexNames[j]);
                            indices.push({name : idx.name,keyPath : idx.keyPath});
                        };
                        stores.push({
                           name: storeName,
                           ...
                           count: store.count(),
                           indices: indices//Array.prototype.slice.apply(store.indexNames)
                        });
                    });
                    return {
                        name: db.name,
